...
def create_link(data, power):  # 建立多項式副程式
...
def sum_link(a, b, power):  # 多項式相加副程式
    i = 0
    ptr = b
    plus = [None]*(power + 1)
    while a != None:  # 判斷多項式1
        if a.exp == b.exp:  # 指數相等，係數相加
            ...
            a = a.next
            i = i+1
    return create_link(plus, power)  # 建立相加結果串列C
...
def main():
    ...
    # c=LinkedList()
    print("原始多項式：\nA=", end='')
    a = create_link(data1, power)  # 建立多項式A
    b = create_link(data2, power)  # 建立多項式B
    print_link(a)  # 列印多項式A
    print("B=", end='')
    print_link(b)  # 列印多項式B
    print("多項式相加結果：\nC=", end='')  # C為A、B多項式相加結果
    print_link(sum_link(a, b, power))  # 列印多項式C
