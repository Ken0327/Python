...
currentvaluepath = './CurrentValue.json'
currentvaluetable = json.load(open(currentvaluepath))
...
def fundValueCrawling():
    try:
        ...
        Messages += fundRand + '\n'
        res = requests.get(
            'https://www.moneydj.com/funddj/yp/yp011001.djhtm?a=TLZ78')
        ...
        Messages += fundUS + '\n'
        res = requests.get(
            'https://www.moneydj.com/funddj/yp/yp011001.djhtm?a=TLZ64')
        soup = BeautifulSoup(res.text, "lxml")
        values_US = []
        ...
        res_USDtoRand = requests.get(
            'https://tw.money.yahoo.com/currency/USDZAR=X')
        res_RandtoTWD = requests.get(
            'https://forex.cnyes.com/currency/ZAR/TWD')
        ...
        # soup.select('.Ta-end remark')
        data = soup_USDtoRand.find_all(
            "div", class_="Ta-end remark")[0].text[0:10]
        # print('最新日期: ' + data)
        currency_UStoRand = float(soup_USDtoRand.find_all(
            "span", class_="Fw-b Fz-27 Lh-27 C-n Pend-20")[0].text)
        currency_RandtoUS = 1 / currency_UStoRand

        currency_RandtoTW = float(soup_RandtoTWD.find_all(
            "div", class_="jsx-685978323 currency-number currency-now")[0].text)
        currency_UStoTW = float(soup_UStoTWD.find_all(
            "div", class_="jsx-685978323 currency-number currency-now")[0].text)
        ...
        valuetotalTW = '*保單價值-台幣: ' + \
            str(unitAll * float(values_Rand[0]) * currency_RandtoTW)
        valuetotalUS = '*保單價值-美金: ' + str(unitAll * float(values_Rand[0]) * currency_RandtoUS) + '-> 美金轉回台幣: ' + str(
            unitAll * float(values_Rand[0]) * currency_RandtoUS * currency_UStoTW)
        affodableunitUS = '*可購買美金保單單位數: ' + \
            str(unitAll * float(values_Rand[0]) *
                currency_RandtoUS / float(values_US[0]))
...
def pushMessages(message):
    try:
        line_bot_api.push_message(
            LINE_CHANNEL_SECRET, TextSendMessage(text=message))
